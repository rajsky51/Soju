...
public class Mixer
{
	...
	public uint256 CompleteMix()
	{
		...
		Dictionary<string, CoinJoinClientContext> cjCtxs = [];
		foreach (var cjManager in CJManagers)
		{
			CoinJoinClientContext cjCtx = cjManager.CreateCoinJoinClientCtx();
			cjCtxs[cjCtx.WalletName] = cjCtx;
		}
		
		Arena.CreateRound();
		Arena.SetRoundStates();
		
		Debug.Assert(Arena.GetRoundStates().Count() == 1);
		
		RoundState roundState = Arena.GetRoundStates().First();
		Debug.Assert(roundState.Phase == Phase.InputRegistration);
		
		ConcurrentBag<(string WalletName, List<InputRegistrationRequestWithId> Requests)> inputRequestBag = [];
		...
		Parallel.ForEach(cjCtxs.Values, cjCtx =>
		{
			try 
			{
				...
				inputRequestBag.Add((cjCtx.WalletName, requests));
			} 
			catch (Exception ex)
			{
		...
		(string WalletName, InputRegistrationRequestWithId RequestWithId)[] inputRequestsWithWalletNames = 
			inputRequestBag
				.SelectMany(pair => pair.Requests.Select(req => (pair.WalletName, req)))
				.ToArray();
		inputRequestsWithWalletNames.Shuffle(wrnd);
		
		Dictionary<string, List<InputRegistrationResponseWithId>> inputRegResponses = [];
		foreach(string walletName in cjCtxs.Keys)
		{
			inputRegResponses[walletName] = [];
		}
		
		foreach (var requestWithWalletId in inputRequestsWithWalletNames)
		{
			string walletName = requestWithWalletId.WalletName;
			...
			try {
				...
				inputRegResponses[walletName].Add(responseWithId);
			}
		...
		ConcurrentDictionary<string, List<AliceClient>> aliceClientsNeedToConfirm = [];
		
		foreach (string walletName in inputRegResponses.Keys)
		{
			CoinJoinClientContext cjCtx = cjCtxs[walletName];
			InputRegistrationResponseHandlingResult handlingResult = cjCtx.CoinJoinClient.HandleInputRegistrationResponses(inputRegResponses[walletName]);
			aliceClientsNeedToConfirm[walletName] = handlingResult.AliceClients;
		}
		Arena.StepInputRegistrationPhase();
		Arena.SetRoundStates();
		
		roundState = Arena.GetRoundStates().First();
		Debug.Assert(roundState.Phase == Phase.ConnectionConfirmation);
		
		ConcurrentBag<(string WalletName, List<ConnectionConfirmationRequestWithId> Requests)> ccRequestBag = [];
		
		sw.Restart();
		Parallel.ForEach(cjCtxs.Values, cjCtx =>
		{
			List<ConnectionConfirmationRequestWithId> requests =  cjCtx.CoinJoinClient.CreateConnectionConfirmationRequests(aliceClientsNeedToConfirm[cjCtx.WalletName]);
			
			ccRequestBag.Add((cjCtx.WalletName, requests));
		});
		sw.Stop();
		Console.WriteLine($"Creating connection confirmation requests took {sw.ElapsedMilliseconds} ms");
		
		(string WalletName, ConnectionConfirmationRequestWithId RequestWithId)[] ccRequestsWithWalletNames = 
			ccRequestBag
			.SelectMany(pair => pair.Requests.Select(req => (pair.WalletName, req)))
			.ToArray();
		ccRequestsWithWalletNames.Shuffle(wrnd);
		
		Dictionary<string, List<ConnectionConfirmationResponseWithId>> ccResponses = [];
		foreach(string walletName in cjCtxs.Keys)
		{
			ccResponses[walletName] = [];
		}
		
		foreach (var requestWithWalletName in ccRequestsWithWalletNames)
		{
			string walletName = requestWithWalletName.WalletName;
			ConnectionConfirmationRequestWithId requestWithId = requestWithWalletName.RequestWithId;
			...
			ccResponses[walletName].Add(responseWithId);
		}
		
		foreach (string walletName in ccResponses.Keys)
		{
			CoinJoinClientContext cjCtx = cjCtxs[walletName];
			
			List<AliceClient> aliceClients = cjCtx.CoinJoinClient.HandleConnectionConfirmationResponses(ccResponses[walletName]);
		...
		foreach (var sigRequest in sigRequestBag.ToArray())
		{
		...
		Arena.SetRoundStates();
		Debug.Assert(Arena.GetRoundStates().First().Phase == Phase.Ended);
		...
		foreach (CoinJoinClientContext cjCtx in cjCtxs.Values)
		{
			...
			Debug.Assert(wallet.KeyChain is not null);
			wallet.KeyChain.TrySetScriptStates(KeyState.Used, cjCtx.WantedOutputs.Select(txOut => txOut.ScriptPubKey));
		}
