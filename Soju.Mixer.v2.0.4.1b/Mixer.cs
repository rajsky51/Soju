...
public class  Mixer : IMixer
{
    ...
    public CoinjoinResult CompleteMix(IEnumerable<IWallet> participatingWallets)
    {
        ...
        {
            CoinJoinClient coinJoinClient = new(wallet.OutputProvider, "foo", CoinJoinCoinSelector.FromWallet(wallet));
            HashSet<DumbCoin> selectedCoins = [];
            try
            {
                selectedCoins = coinJoinClient.StartCoinJoin(wallet, RoundParams).ToHashSet();
