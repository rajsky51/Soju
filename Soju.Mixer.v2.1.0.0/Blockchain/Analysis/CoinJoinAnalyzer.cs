...
public class CoinjoinAnalyzer
{
	...
	public CoinjoinAnalyzer(DumbTransaction transaction)
	{
		...
		AnalyzedTransactionPrevOuts = AnalyzedTransaction.Inputs.SelectMany(pair => pair.Value.Select(input => input.OutPoint)).ToHashSet();
	}
	...
	private Dictionary<DumbCoin, double> CachedInputSanctions { get; } = new();
	
	public DumbTransaction AnalyzedTransaction { get; }
	...
	public double ComputeInputSanction(DumbCoin transactionInput, WalletId walletId, AggregationFunction aggregationFunction)
	{
		double ComputeInputSanctionHelper(DumbCoin transactionOutput, int recursionDepth = 1)
		{
			...
			double sanction = -ComputeAnonymityContribution(transactionOutput, walletId, AnalyzedTransactionPrevOuts);
