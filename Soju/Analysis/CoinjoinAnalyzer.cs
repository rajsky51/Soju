...
public class CoinjoinAnalyzer
{
	...
	public CoinjoinAnalyzer(DumbTransaction transaction)
	{
		_analyzedTransactionPrevOuts = transaction.Inputs.SelectMany(pair => pair.Value.Select(input => input.OutPoint)).ToHashSet();
	}
	...
	private readonly Dictionary<DumbCoin, double> _cachedInputSanctions = new();

	public double ComputeInputSanction(WalletVirtualInput virtualInput, WalletId walletId, AggregationFunction aggregationFunction)
		=> virtualInput.Coins.Select(x => ComputeInputSanction(x, walletId, aggregationFunction)).Max();

	public double ComputeInputSanction(DumbCoin transactionInput, WalletId walletId, AggregationFunction aggregationFunction)
	{
		double ComputeInputSanctionHelper(DumbCoin transactionOutput, int recursionDepth = 1)
		{
			...
			DumbTransaction transaction = transactionOutput.Transaction;
			double sanction = -ComputeAnonymityContribution(transactionOutput, walletId, _analyzedTransactionPrevOuts);
			...
			IReadOnlyCollection<DumbCoin> walletInputs = transaction.Inputs[walletId];
	...
	/// </summary>
	public static double ComputeAnonymityContribution(DumbCoin transactionOutput, WalletId walletId, HashSet<OutPoint>? relevantOutpoints = null)
	{
		DumbTransaction transaction = transactionOutput.Transaction;
		
		List<WalletVirtualOutput> walletVirtualOutputs = [];
		List<ForeignVirtualOutput> foreignVirtualOutputs = [];

		foreach (var outputs in transactionOutput.Transaction.Outputs)
		{
			if (outputs.Key == walletId)
				walletVirtualOutputs
				.AddRange(outputs.Value
					.Select(x => new WalletVirtualOutput(x.KeyId, (HashSet<DumbCoin>)[x])));
			else foreignVirtualOutputs
				.AddRange(outputs.Value
